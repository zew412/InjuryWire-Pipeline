...
async function pollTweetAPI() {
  ...
  try {
    const response = await axios.get('https://api.tweetapi.io/twitter/tweet/search', {
      headers: { 'X-API-Key': TWEETAPI_KEY },
      params:  { query, maxResults: 100, tweetFields: 'created_at,author_id' },
      timeout: 15000,
    });
    ...
    for (const tweet of tweets) {
      ...
      const conf    = calcConfidence(reporter, status);
...
// Simple API key check — set API_KEY in Railway env vars
