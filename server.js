...
function extractBodyPart(text) {
  const t = text.toLowerCase();
  // Check parenthetical format first e.g. "Tyler Herro (quad)"
  const parenMatch = t.match(/\(([^)]+)\)/);
  if (parenMatch) {
    const parts = ['ankle','knee','quad','hamstring','achilles','back','shoulder','hip','groin','calf','foot','wrist','elbow','hand','concussion','illness','shin','toe','finger','neck','chest','rib','abdomen','tendon','ligament','meniscus','acl','mcl','pcl'];
    const found = parts.find(p => parenMatch[1].includes(p));
    if (found) return found;
  }
  const parts = ['ankle','knee','quad','hamstring','achilles','back','shoulder','hip','groin','calf','foot','wrist','elbow','hand','concussion','illness','shin','toe','finger','neck','chest','rib','tendon','ligament','meniscus','acl','mcl'];
  return parts.find(p => t.includes(p)) || 'undisclosed';
}

function extractPlayer(text) {
  const patterns = [
    // "Tyler Herro (quad) is out"
    /^([A-Z][a-z']+(?:\s+[A-Z][a-z']+){1,3})\s*\(/,
    // "Tyler Herro is out tonight"
    /^([A-Z][a-z']+(?:\s+[A-Z][a-z']+){1,3})\s+(?:is|will|won'?t|has|was|did)/,
    // "ruled out: Tyler Herro" or similar
    /(?:ruled out|out tonight|questionable|doubtful|probable|gtd)[:\s]+([A-Z][a-z']+(?:\s+[A-Z][a-z']+){1,3})/i,
    // mid-sentence "... Tyler Herro is questionable"
    /([A-Z][a-z']+(?:\s+[A-Z][a-z']+){1,3})\s+(?:is|will not|won'?t)\s+(?:play|out|questionable|doubtful|available)/i,
    // "Tyler Herro listed as"
    /([A-Z][a-z']+(?:\s+[A-Z][a-z']+){1,3})\s+(?:listed|ruled|scratched|inactive|out)/i,
    // "Tyler Herro - ankle" or "Tyler Herro — knee"
    /([A-Z][a-z']+(?:\s+[A-Z][a-z']+){1,3})\s*(?:–|—|-)\s*(?:ankle|knee|quad|hamstring|achilles|back|shoulder|hip)/i,
  ];
  for (const p of patterns) {
    const m = text.match(p);
    if (m?.[1]?.length > 3 && !m[1].match(/^(The|This|He|She|They|We|His|Her|Their|Per|Via|From|With|For|Breaking)/i)) {
      return m[1].trim();
    }
  }
  return null;
